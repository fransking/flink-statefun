...
public class SocketClientSink<T> implements Sink<T> {
    ...
    public SocketClientSink(String hostName, int port, SerializationSchema<T> schema, int maxNumRetries, boolean autoflush) {
    ...
    public SinkWriter<T> createWriter(WriterInitContext context) throws IOException {
        return new SocketClientSinkWriter(this.hostName, this.port, this.schema, this.maxNumRetries, this.autoflush);
    }
    ...
    private class SocketClientSinkWriter implements SinkWriter<T> {
        ...
        public SocketClientSinkWriter(String hostName, int port, SerializationSchema<T> schema, int maxNumRetries, boolean autoflush) throws IOException {
        ...
        public void write(T value, SinkWriter.Context context) throws IOException, InterruptedException {
            ...
            try {
                ...
                if (this.maxNumRetries == 0) {
                    throw new IOException("Failed to send message '" + String.valueOf(value) + "' to socket server at " + this.hostName + ":" + this.port + ". Connection re-tries are not enabled.", e);
                }

                LOG.error("Failed to send message '" + String.valueOf(value) + "' to socket server at " + this.hostName + ":" + this.port + ". Trying to reconnect...", e);
                synchronized(this.lock) {
                    ...
                    if (this.isRunning) {
                        throw new IOException("Failed to send message '" + String.valueOf(value) + "' to socket server at " + this.hostName + ":" + this.port + ". Failed after " + retries + " retries.", lastException);
                    }
                }
            }
        }
